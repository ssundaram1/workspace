...
public class BinaryTree {
  ...
  public int heightv2(Node node) {
    if (node == null) {
      ...
      return(MathProbs.max(leftHeight, rightHeight) + 1);
    }
  }
  ...
  private int maxDepth(Node node) {
    if (node == null) {
      ...
      // use the larger + 1
      return(MathProbs.max(lDepth, rDepth) + 1);
    }
  }
  ...
  private boolean isBTBalancedv2(Node node) {
    ...
    return(MathProbs.abs(heightv2(node.left) - heightv2(node.right)) <= 1);
  ...
  private boolean isBTBalanced(Node node) {
    ...
    return(isBTBalanced(node.left) && isBTBalanced(node.right) && (MathProbs.abs(maxDepth(node.left) - maxDepth(node.right)) <= 1));
  ...
  int diameter(Node root) {
     ...
     */
    return MathProbs.max(lheight + rheight + 1, MathProbs.max(ldiameter, rdiameter));
  ...
  static int height(Node node) {
     ...
     */
    return(1 + MathProbs.max(height(node.left), height(node.right)));
  }
